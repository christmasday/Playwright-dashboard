        ...
        <div className="bg-[#1a1a22] border border-[#20202a] rounded-xl p-6">
          ...
          <div className="grid grid-cols-1 sm:grid-cols-2 md:grid-cols-3 gap-4">
            {screenshots.map((artifact) => {
              const isImageFailed = failedImages[artifact.id];
              return (
                <div
                  key={artifact.id}
                  onClick={() => !isImageFailed && handleImageClick(artifact)}
                  className={`group relative bg-[#0e0e13] border border-[#20202a] rounded-xl overflow-hidden transition-all ${
                    isImageFailed ? 'cursor-default' : 'cursor-pointer hover:border-[#3b82f6]/50'
                  }`}
                >
                  <div className="relative h-48 overflow-hidden bg-[#08080a] flex items-center justify-center">
                    {isImageFailed ? (
                      <div className="flex flex-col items-center justify-center p-4 text-center">
                        <div className="w-10 h-10 rounded-xl bg-red-500/10 border border-red-500/20 text-red-400 flex items-center justify-center mb-2">
                          <i className="fas fa-image text-sm opacity-80"></i>
                        </div>
                        <p className="text-xs font-medium text-[#f4f4f7] truncate max-w-full px-2">{artifact.name}</p>
                        <p className="text-[11px] text-[#9a9aa5] mt-1">Snapshot file unavailable or expired</p>
                      </div>
                    ) : (
                      <>
                        <img
                          ...
                          onError={() => {
                            setFailedImages((prev) => ({ ...prev, [artifact.id]: true }));
                          }}
                        />
      ...
      {videos.length > 0 && (
        <div className="bg-[#1a1a22] border border-[#20202a] rounded-xl p-6">
          ...
          <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
            {videos.map((artifact) => {
              const isVideoFailed = failedVideos[artifact.id];
              return (
                <div key={artifact.id} className="bg-[#0e0e13] border border-[#20202a] rounded-xl overflow-hidden p-3">
                  {isVideoFailed ? (
                    <div className="w-full h-56 rounded-lg bg-[#14141b] border border-[#20202a] flex flex-col items-center justify-center p-4 text-center">
                      <div className="w-10 h-10 rounded-xl bg-amber-500/10 border border-amber-500/20 text-amber-400 flex items-center justify-center mb-2">
                        <i className="fas fa-video-slash text-sm opacity-80"></i>
                      </div>
                      <p className="text-xs font-medium text-[#f4f4f7] truncate max-w-full px-2">{artifact.name}</p>
                      <p className="text-[11px] text-[#9a9aa5] mt-1">Video recording unavailable or expired</p>
                    </div>
                  ) : (
                    <video
                      src={getAuthUrl(artifact.url)}
                      ...
                      preload="metadata"
                      crossOrigin="anonymous"
                      className="w-full h-56 rounded-lg bg-black object-contain"
                      onError={() => {
                        setFailedVideos((prev) => ({ ...prev, [artifact.id]: true }));
                      }}
                    />
                  )}
